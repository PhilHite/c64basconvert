# c64basconvert v1.01 (macOS)

A macOS command line tool that converts a plain text C64 BASIC `.BAS` file into a properly structured C64 BASIC binary `.PRG` file, ready to load directly into a C64 emulator such as VICE or Virtual64.
...
```bash
c64basconvert INPUTFILE.BAS OUTPUTFILE.PRG
```

Example:

```bash
c64basconvert PROGRAM.BAS PROGRAM.PRG
```

The tool will report how many lines were converted and confirm the `$0801` header was written successfully.
...
The C64 stores BASIC programs in a specific binary format in memory starting at address `$0801`. A plain text `.BAS` file needs to be converted into this format before the C64 can run it. The tool performs the following steps:
...
**5. Trailing newline**
If the source `.BAS` file does not end with a newline character, one is automatically appended before conversion. This ensures the last line is always processed correctly regardless of how the file was saved.
